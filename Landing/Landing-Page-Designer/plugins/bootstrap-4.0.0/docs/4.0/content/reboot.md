...
layout: docs
title: Reboot
desc: Reboot, a collection of element-specific CSS changes in a single file, kickstart Bootstrap to provide an elegant, consistent, and simple baseline to build upon.
group: content
...
For simpler styling, clear hierarchy, and better spacing, desc lists have updated `margin`s. `<dd>`s reset `margin-left` to `0` and add `margin-bottom: .5rem`. `<dt>`s are **bolded**.

<div class="bd-example">
{% markdown %}
<dl>
  <dt>Desc lists</dt>
  <dd>A desc list is perfect for defining terms.</dd>
  <dt>Euismod</dt>
